...
{
    if (my_path_id() != PATH_AVATAR_OF_WEST_OF_LOATHING && !($classes[snake oiler,beanslinger,cow puncher] contains my_class())) return;
    ...
    if ($skill[Beancannon].have_skill() && in_ronin())
    {
        ...
        if (banish_count > 0 || !in_ronin())
        {
            ...
            resource_entries.listAppend(ChecklistEntryMake("__skill beancannon", url, ChecklistSubentryMake(title, "", description), 8).ChecklistEntryTagEntry("banish").ChecklistEntrySetIDTag("West Loathing path beancannon banish"));
        }
    }
