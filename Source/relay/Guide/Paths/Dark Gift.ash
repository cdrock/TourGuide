...
void PathDarkGiftGenerateResource(ChecklistEntry [int] resource_entries)
{
    ...
    if (banishes_left > 0 && lookupSkill("Baleful Howl").skill_is_usable())
    {
        ...
        resource_entries.listAppend(ChecklistEntryMake("__skill Baleful Howl", url, ChecklistSubentryMake(pluralise(banishes_left, "baleful howl", "baleful howls"), "", description), 0).ChecklistEntryTagEntry("banish").ChecklistEntrySetIDTag("Dark gyffte path baleful howl banish"));
    }
