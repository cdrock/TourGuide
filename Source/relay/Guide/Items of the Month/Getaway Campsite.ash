...
void IOTMGetawayCampsiteGenerateResource(ChecklistEntry [int] resource_entries)
{
    ...
    if (cloud_buffs_left > 0) { // && lookupEffect("That's Just Cloud-Talk, Man").have_effect() == 0)
        ...
        resource_entries.listAppend(ChecklistEntryMake("__item Newbiesport&trade; tent", "place.php?whichplace=campaway", ChecklistSubentryMake("Cloud-talk buff obtainable", "", description), 0).ChecklistEntryTagEntry("getaway campsite resources").ChecklistEntrySetIDTag("Getaway campsite cloud talk"));
    }
    if (smile_buffs_left > 0) { // && lookupEffect("That's Just Cloud-Talk, Man").have_effect() == 0)
        ...
        resource_entries.listAppend(ChecklistEntryMake("__item Newbiesport&trade; tent", "place.php?whichplace=campaway", ChecklistSubentryMake(pluralise(smile_buffs_left, "smile buff", "smile buffs") + " obtainable", "20 turns", description), 5).ChecklistEntryTagEntry("getaway campsite resources").ChecklistEntrySetIDTag("Getaway campsite sign smiles"));
    }
    if (firewood.have() && __misc_state["in run"]) {
        ...
        resource_entries.listAppend(ChecklistEntryMake("__item Newbiesport&trade; tent", "shop.php?whichshop=campfire", ChecklistSubentryMake(pluralise(firewood), "", description), 5).ChecklistEntryTagEntry("getaway campsite resources").ChecklistEntrySetIDTag("Getaway campsite firewood"));
    }
}
