...
void IOTMWitchessGenerateResource(ChecklistEntry [int] resource_entries)
{
    ...
    if (get_property_int("_witchessFights") < 5)
    {
        ...
        resource_entries.listAppend(ChecklistEntryMake(image_name, "campground.php?action=witchess", ChecklistSubentryMake(pluralise(fights_remaining, "witchess fight", "witchess fights"), "", description)).ChecklistEntryTagEntry("daily free fight").ChecklistEntrySetIDTag("Witchess set free fights"));
    }
