...
void IOTMNewYouGenerateResource(ChecklistEntry [int] resource_entries)
{
    if (__misc_state["in run"] && in_ronin())
    {
        ...
        foreach it in affirmation_effects
        {
            ...
            {
            	resource_entries.listAppend(ChecklistEntryMake("__item " + it, "", subentry).ChecklistEntryTagEntry("free instakill").ChecklistEntrySetIDTag("New you affirmation win-lose free kill"));
            }
            else if (it == lookupItem("Daily Affirmation: Be a Mind Master"))
            {
                resource_entries.listAppend(ChecklistEntryMake("__item " + it, "", subentry).ChecklistEntryTagEntry("banish").ChecklistEntrySetIDTag("New you affirmation mind master banish"));
            }
            else
